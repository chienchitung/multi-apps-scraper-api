...
- 每個應用程式最多抓取 300 筆最新評論（iOS 和 Android 各 150 筆）
- 自動語言檢測（支援中文和英文）
- 評論依日期排序（從新到舊）
- 包含評分、開發者回覆等完整資訊
...
4. 環境變數設定：
   - `PORT`: 設定服務埠口（預設 8000）
   - `SERPAPI_KEY`：SerpApi 的 API Key（**iOS 評論抓取必填**）。iOS 評論透過 SerpApi 的 `apple_reviews` engine 抓取（原本的 iTunes RSS Feed 對部分 App/地區會不穩定回傳空結果，已移除）。未設定時 iOS 評論會回傳空陣列，不影響 Android 評論抓取。
...
- 評論抓取限制：每個應用程式最多抓取 300 筆評論（iOS 和 Android 各 150 筆）
- 評論排序：依照日期從新到舊排序
